...
class mfh_coatt_Med_block(nn.Module):
    def __init__(self, opt, embedding_matrix):
        super(mfh_coatt_Med_block, self).__init__()
        self.opt = opt
        ...
        # block fusion
        self.Block_qc_etm = Block(input_dims=[4, 10], output_dim=10)
        self.Block_q_qc_etm = Block(input_dims=[2048, 10], output_dim=1024)
        self.Block_q_i = Block(input_dims=[4096, 1024], output_dim=2048)
        ...
        self.Classifier = nn.Linear(2048, opt.ALL_CLASS_NUM)
        ...
        self.ans_LSTM = nn.LSTM(input_size=2048, hidden_size=opt.NUM_OUTPUT_UNITS, num_layers=1, batch_first=False)
    ...
    def forward(self, ivec, q_indices, qc, etm_topic, mode):
        ...
        BLOCK fusion
        '''
        # fuse question category and etm topic distribution
        block_qc_etm = self.Block_qc_etm([qc, etm_topic])
        block_q_qc_etm = self.Block_q_qc_etm([q_feat_resh, block_qc_etm])
        block_q_i = self.Block_q_i([iatt_feature_concat, block_q_qc_etm]) # b_size x 2048
        ...
        '''
        # classifier
        classification = self.Classifier(block_q_i)
        ...
        input_for_ans_lstm = torch.unsqueeze(block_q_i, dim=0) # 1 x b_size x 2048
        # print(input_for_ans_lstm.size())
        padding = torch.zeros(self.opt.MAX_WORDS_IN_ANSWER-1, self.batch_size, 2048).cuda().float()
...
if __name__ == "__main__":
    ...
    model = mfh_coatt_Med_block(opt, embedding_matrix)
    model.cuda()
