...
# load_path = os.path.join(root_path, "models/pretrained/etm_tuned_densenet.pt")
...
class mfh_coatt_Med_emb(nn.Module):
    def __init__(self, opt, embedding_matrix):
        super(mfh_coatt_Med_emb, self).__init__()
        self.opt = opt
        ...
        # fine tune last x conv blocks of ResNet-152
        # last conv block
        for param in self.img_feature_extr[7][2].parameters():
            param.requires_grad = True
        # last 2 blocks
        for param in self.img_feature_extr[7][1].parameters():
            param.requires_grad = True
        ...
        # input_for_last_layer = int(opt.MFB_OUT_DIM*2/opt.MAX_WORDS_IN_ANSWER)
    ...
    def forward(self, ivec, q_indices, qc, etm_topic, mode):
        ...
        # print(mfb_o23_l2.size())

        # prediction = self.Linear_predict(mfb_o23_l2)
        # prediction = prediction.view(self.batch_size, self.opt.MAX_WORDS_IN_ANSWER, self.opt.NUM_OUTPUT_UNITS)
        ...
        # sampling method to produce answer
        prediction = self.Linear_predict(mfb_o23_l2)
        prediction = prediction.view(self.batch_size, self.opt.MAX_WORDS_IN_ANSWER, self.opt.NUM_OUTPUT_UNITS)
        prediction = F.log_softmax(prediction, dim=2)
...
if __name__ == "__main__":
    ...
    model = mfh_coatt_Med_emb(opt, embedding_matrix)
    img = torch.randn(8, 3, 224, 224)
    q = torch.randint(-1, 30, size=(8, 10))
    qc = torch.randn(8, 4)
    # qc = torch.randint(2, (8, 4), dtype=torch.DoubleTensor)
    etm_topic = torch.randn(8, 10)
